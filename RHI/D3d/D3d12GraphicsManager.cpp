...
HRESULT ZetaEngine::D3d12GraphicsManager::CreateRenderTarget()
{
	...
	{
		if (FAILED(hr = m_pSwapChain->GetBuffer(i, IID_PPV_ARGS(&m_pRenderTagets[i]))))
		{
			break;
		}
		m_pDev->CreateRenderTargetView(m_pRenderTagets[i], nullptr, rtvHandle);
...
HRESULT ZetaEngine::D3d12GraphicsManager::CreateGraphicsResources()
{
	...
	// fill the swap chain desciption struct
	...
	scd.Stereo = FALSE;
	scd.SampleDesc.Count = 1;							// multi-samples cna not be used when in SwapEffect sets to
														// DXGI_SWAP_EFFECT_FLOP_DISCARD
	scd.SampleDesc.Quality = 0;							// multi-samples can not be used when in SwapEffects sets to
