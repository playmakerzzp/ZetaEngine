...
namespace ZetaEngine
{
    Point2DList BottomFlatTriangleInterpolation(const Point2D& v1, const Point2D& v2, const Point2D& v3)
    {
        ...
        assert(v2[1] == v3[1]); // the bottom must flat;
    ...
    {
        Point2DList result; // the top must be flat
        assert(v1[1] == v2[1]); // v3 must be the bottom vertex
    ...
    Point2DList BaryCentricTriangleInterpolation(const Point2D& v1, const Point2D& v2)
    {
        Point2DList result;
        ...
        auto maxY = max({v1[1], v2[1], v3p[1]});

        Vector2f edge_1_2 = v2 - v1;
        Vectoe2f edge_1_3 = v3 - v1;
